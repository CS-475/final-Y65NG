...
inline GPathBuilder strokeTo(GPathBuilder path, GPoint p1, GPoint p2, float width) {
    float radius = width / 2;

    GVector norm = {(p2.y - p1.y), -(p2.x - p1.x)};
    norm = norm * (1 / norm.length()) * radius;
